...
void OnlyCompressMinorAxis(BoundaryE* be, float rate) {
	be->b = be->b * rate;
	be->aspect_ratio = be->a / be->b;
	be->sol = BESolver(be->a, be->b);
}
...
struct Simulation{
	...
	Simulation(bool output_init_data) {
		...
		compression_ratio = (1 - MAX_COMPRESSION_B / BOUNDARY_B);
	...
	void simulate(float scalar_radius) {
		...
		*/
		...
		while (state_info.state->boundary->scalar_radius > END_BOUNDARY_B) {
			...
			// "set" the boundary radius, rather than substract.
			// state_info.state->boundary->setScalarRadius(init_radius - t * this->meta->boundary_compression_rate);
			OnlyCompressMinorAxis(state_info.state->boundary, compression_ratio);
			double tc;
			RecordTime(tc,
...
#endif
			std::cout << "compression: " << t <<
				",\t minor axis: " << state_info.state->boundary->b <<
				",\t iterations: " << data.iterations <<
				",\t energy: " << data.energy << std::endl;
