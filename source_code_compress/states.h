...
struct StateInfo{
	...
	*/
	...
	Eigen::VectorXf* _sphere_space = NULL;		// access variables start with underline by calling corresponding functions
	Grid* _grid = NULL;					
	...
	Eigen::VectorXf* sphereSpace() {
		if (this->_sphere_space == NULL) {
			this->_sphere_space = new Eigen::VectorXf(transformer->lift(*(this->state->q)));
		}
