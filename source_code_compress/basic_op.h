...
template<int m, int N, typename bt>
Eigen::VectorXf _calForce_from_dist(DistInfo<m* N>* rs, SphereAssembly<m>* sa) {
	...
	Eigen::VectorXf forces_pp = temp.rowwiseSumAsym().toVector();
	// particle-wall force
	Eigen::VectorXf forces_pw = rs->distpw->apply(singleForcePW).toVector();
	Eigen::VectorXf forces = forces_pp + forces_pw;
	return forces;
}
...
Vecf<3 * N>* CalGradient(StateInfo<m, N, bt>& state_info) {
	if (state_info.gradient == NULL) {
		Eigen::VectorXf temp = _calForce_from_dist<m, N, bt>(Collision(state_info), state_info.state->sa);
